...
public class XML {
    ...
    public static void iterateChilds(Node node, BiConsumer<String, Node> consumer){
        for (Node el : new XML.IterableNodeList(node.getChildNodes())) {
            if (el.getNodeType() == Node.TEXT_NODE || el.getNodeType() == Node.COMMENT_NODE) continue;
            consumer.accept(el.getNodeName().toLowerCase(), el);
        }
    }
